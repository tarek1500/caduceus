		...
		<div class="col">
			<div class="card">
				<div class="card-header">
					<a href="{{ route('appointments.index') }}">Appointments</a> - {{ $appointment->pain->name }}
						...
						@if ($appointment->status === App\Enums\AppointmentStatus::APPROVED && $appointment->date > now())
							<div class="form-group row mb-0">
								<div class="col-md-8 offset-md-4">
									<a class="btn btn-danger" href="{{ route('appointments.update', $appointment->id) }}" onclick="event.preventDefault(); document.getElementById('cancel-appointment').submit();">Cancel</a>
								</div>
							</div>
						@endif
					</form>
					@if ($appointment->status === App\Enums\AppointmentStatus::APPROVED && $appointment->date > now())
						<form id="cancel-appointment" action="{{ route('appointments.update', $appointment->id) }}" method="POST" style="display: none;">
							@csrf
							@method('PUT')
						</form>
					@endif
